...
if opt.resume_epoch < 0  :
    ...
    net_d_model_out_path = "checkpoint/{}_{}/netD_model_epoch_{}.pth".format(opt.dataset,opt.G_model, resume_epoch)
    netG = torch.load(net_g_model_out_path)
    netD = torch.load(net_d_model_out_path)
else :
...
def train(epoch):
    for iteration, batch in tqdm(enumerate(training_data_loader, 1), total=len(training_data_loader)):
        # forward
        input_masked_cpu , target_cpu , input_cpu= batch[7], batch[1] , batch[3]
        ...
        fake_b = netG(real_masked_a)
        pred_fake = netD(fake_b)
        #D_real_loss = -torch.mean(pred_fake)
        loss_g_gan = -torch.mean(pred_fake) #BCE_loss(pred_fake, y_real_)

        loss_g_vgg = generator_criterion(fake_b, real_b) * 10
        loss_g_l1 = criterionL1(fake_b, real_b) * 1000
...
def test(epoch):
    ...
    for batch in testing_data_loader:
        input, target, input_masked = Variable(batch[3], volatile=True), Variable(batch[1], volatile=True), Variable(batch[7], volatile=True)
        if opt.cuda:
...
log_dir = os.path.join('runs_stage2', opt.dataset+"_"+str(opt.resume_epoch)+"_"+current_time)
